...
if %ERRORLEVEL% NEQ 0 (
    echo.
    echo ❌ 테이블 구조를 확인할 수 없습니다.
    echo.
    echo 확인사항:
    echo   1. restaurants 테이블이 존재하는지 확인
    echo   2. 비밀번호가 맞는지 확인
    echo   3. 네트워크 연결 확인
    echo.
    pause
...
REM 테이블 생성 SQL 확인
mysql -h %CLOUD_SQL_IP% -u root -p%CLOUD_DB_PASSWORD% %DB_NAME% -e "SHOW CREATE TABLE restaurants\G" 2>nul
